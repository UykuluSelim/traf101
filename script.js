...
const trafficSigns = [
    ...
    },
    {
        ...
        question: 'What is the mistake made by the driver who does not use the turn signal in this video?',
        choices: ['Exceeding the speed limit', 'Changing lanes without signaling', 'Blocking the pedestrian crossing', 'Honking unnecessarily'],
        correctAnswer: 'Changing lanes without signaling',
        timeLimit: 45
    },
    {
        mediaType: 'video',
        videoSource: 'images/video2.mp4',
        question: 'What is the main cause of the accident in this video?',
        choices: ['High tire pressure', 'Insufficient following distance', 'A loose seat belt', 'Fog lights being on'],
        correctAnswer: 'Insufficient following distance',
        timeLimit: 45
    },
    {
        mediaType: 'video',
        videoSource: 'images/video3.mp4',
        question: 'What mistake is the driver making?',
        choices: ['Changing lanes', 'Using high beams', 'Driving without turning on the headlights', 'Driving at low speed'],
        correctAnswer: 'Driving without turning on the headlights',
        timeLimit: 45
    },
    {
        mediaType: 'video',
        videoSource: 'images/video4.mp4',
        question: 'What is the biggest risk of this behavior?',
        choices: ['Increasing fuel consumption', 'Giving way to other vehicles', 'Increasing the risk of an accident due to distraction', 'Causing the tires to wear out faster'],
        correctAnswer: 'Increasing the risk of an accident due to distraction',
        timeLimit: 45
    },
    {
        mediaType: 'video',
        videoSource: 'images/video5.mp4',
        question: 'Which traffic rule has the driver violated?',
        choices: ['Speed limit', 'Parking rules', 'Traffic light violation', 'Lane discipline'],
        correctAnswer: 'Traffic light violation',
        timeLimit: 45
    }
];
...
function initAudioContext() {
    ...
    // 2. FIX: Check if the browser put it to sleep (suspended) and wake it up
...
function playCorrectSound() {
    ...
    // Second note for the "Ding-Dong" effect
    setTimeout(() => {
...
function startGame() {
    ...
    gameContainer.classList.add('fade-out'); // Briefly hide the game container for a transition effect
    ...
    setTimeout(() => {
        ...
        // --- NEW: Footer Swipe Down Animation ---
        // This adds the CSS class that pushes the footer off-screen smoothly
        if (typeof footer !== 'undefined' && footer) {
            footer.classList.add('slide-down');
        } else {
            // Safety fallback: Find the footer manually if the variable is missing
            const footerElement = document.querySelector('.footer');
            if (footerElement) footerElement.classList.add('slide-down');
        }
        // ----------------------------------------
        ...
        // Bring the game container back into view
        gameContainer.classList.remove('fade-out');
        
        // Load the first question
        loadQuestion();
    }, 500); // Wait 500ms for the fade-out animation to finish
}
